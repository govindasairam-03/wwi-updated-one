...
import { applications } from "./firestore";
...
export function JobApplicationForm({ role, jobId, onSuccess }: JobApplicationFormProps) {
  ...
  return (
    <div className="bg-background rounded-3xl border border-border overflow-hidden flex flex-col min-h-[74vh] shadow-sm">
      <div className="flex justify-between items-start p-6 pb-4 border-b border-border bg-gradient-to-r from-background to-card">
        <div>
          <div className="text-xs uppercase tracking-wider text-muted-foreground">Apply for</div>
          <h3 className="text-lg md:text-xl font-bold mt-1">{role}</h3>
          <p className="mt-2 text-sm text-muted-foreground max-w-2xl">Complete the form step by step. Each section is sized to stay readable and easy to scan on any screen.</p>
        </div>
      </div>
      ...
      <div className="px-6 pt-5">
        <div className="flex items-center gap-2 overflow-x-auto pb-2">
          {[1, 2, 3].map((n) => (
            <div key={n} className="min-w-0 flex-1 flex items-center gap-2">
              <div className={`w-7 h-7 grid place-items-center rounded-full text-xs font-semibold transition-colors ${step >= n ? "bg-foreground text-background" : "bg-secondary text-muted-foreground"}`}>
      ...
      <div className="p-6 overflow-y-auto flex-1">
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {step === 2 && (
            <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {(["Fresher", "Experienced"] as const).map((m) => (
                    <button key={m} type="button" onClick={() => setS2({ ...s2, experienceType: m })}
                      className={`px-4 py-2 rounded-full text-sm border transition-colors whitespace-nowrap ${s2.experienceType === m ? "bg-foreground text-background border-foreground" : "bg-card border-border hover:bg-secondary"}`}>{m}</button>
                  ))}
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {(["Remote", "Hybrid", "Onsite"] as const).map((m) => (
                    <button key={m} type="button" onClick={() => setS2({ ...s2, workMode: m })}
                      className={`px-4 py-2 rounded-full text-sm border transition-colors whitespace-nowrap ${s2.workMode === m ? "bg-foreground text-background border-foreground" : "bg-card border-border hover:bg-secondary"}`}>{m}</button>
                  ))}
                </div>
              </div>
            </motion.div>
      ...
      <div className="p-6 pt-4 border-t border-border flex justify-between gap-3">
        <button type="button" onClick={step === 1 ? onSuccess : back} disabled={submitting}
  ...
  return (
    <label className="block min-w-0">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)}
        className="w-full bg-card border border-border rounded-xl px-3 py-2.5 text-sm outline-none focus:border-foreground transition-colors" />
    </label>
  );
}
  ...
  return (
    <label className="block min-w-0">
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className="w-full bg-card border border-border rounded-xl px-3 py-2.5 text-sm outline-none focus:border-foreground">
